...
def _current():
    ...
    return _bare(
        JSONRPCClient,
        _calls={}, _jobs=defaultdict(dict), _jobs_lock=Lock(),
        _jobs_watching=False, _new_style_jobs=True,
        _event_callbacks=defaultdict(list), _set_options_call=None,
        _call_timeout=5, _log_py_exceptions=False, _py_exceptions=False,
    )
    ...
    return {
        ...
        'params': {
            ...
            'fields': {
                ...
                'state': state,
                'progress': {'percent': 100 if state == 'SUCCESS' else 0, 'description': '', 'extra': None},
                'result': result,
                'error': None,
                'exception': None,
                'exc_info': None,
...
class _FakeServer:
    ...
    def send(self, data):
        ...
        if method == 'core.subscribe':
            ...
            self.next_job_id += 1
            self.client._recv(_job_update('added', job_id, 'WAITING', message_ids=[data['id']]))
            self.client._recv(_job_update('changed', job_id, 'RUNNING', message_ids=[data['id']]))
            self.client._recv(_job_update('changed', job_id, 'SUCCESS', message_ids=[data['id']],
...
class TestForeignJobsAreForgotten(unittest.TestCase):
    ...
    def test_finished_jobs_are_dropped(self):
        ...
        for job_id in range(1, 1001):
            ...
            client._recv(_job_update('changed', job_id, 'SUCCESS', result=['sel entry'] * 100))

        self.assertEqual(len(client._jobs), 0, f'{len(client._jobs)} finished jobs are still held')
...
class TestOwnJobsStillWork(unittest.TestCase):
    ...
    def test_job_that_finishes_before_wait_returns_its_result(self):
        ...
        self.assertEqual(client.call('pool.scrub.scrub', 1, 'START', job=True), 'scrub done')
        self.assertEqual(len(client._jobs), 0)
...
def _legacy():
    ...
    return _bare(
        LegacyClient,
        _calls={}, _jobs=defaultdict(dict), _jobs_lock=Lock(), _jobs_watching=True,
        _event_callbacks=defaultdict(list), _pings={},
        _call_timeout=5, _log_py_exceptions=False, _py_exceptions=False,
    )
...
def _legacy_fields(job_id, state, result=None):
    return {
        'id': job_id, 'method': 'ipmi.sel.elist', 'state': state, 'result': result,
        'progress': {'percent': 100 if state == 'SUCCESS' else 0, 'description': '', 'extra': None},
        'error': None, 'exception': None, 'exc_info': None,
    }
...
class _FakeLegacyServer:
    ...
    def send(self, data):
        ...
        if method in self.jobs:
            ...
            self.client._jobs_callback('ADDED', fields=_legacy_fields(job_id, 'WAITING'))
            self.client._jobs_callback('CHANGED', fields=_legacy_fields(job_id, 'SUCCESS', self.jobs[method]))
        else:
            self.client._recv({'id': data['id'], 'msg': 'result', 'result': f'{method} result'})
...
class TestLegacyForeignJobsAreForgotten(unittest.TestCase):
    ...
    def test_finished_jobs_are_dropped(self):
        ...
        for job_id in range(1, 1001):
            ...
            client._jobs_callback('CHANGED', fields=_legacy_fields(job_id, 'SUCCESS', ['sel entry'] * 100))
